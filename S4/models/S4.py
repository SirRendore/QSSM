    ...
    def __init__(
        ...
        for _ in range(n_layers):
            self.s4_layers.append(
                S4Block(d_model, d_state=d_state, dropout=dropout, transposed=True, lr=min(0.001, lr), mode="diag", init="diag-lin", bidirectional=False, disc="zoh", real_transform="exp")
            )
            self.norms.append(nn.LayerNorm(d_model) if prenorm or postnorm else nn.Identity())
